...
import {
  Loader2, Plus, X, Save, Lightbulb, ChevronUp, ChevronDown,
  MessageSquare, User, Book, CreditCard, Box, Image,
  Edit3, Trash2,
} from "lucide-react";
import { getHints, createHint, updateHint, deleteHint, updateHintOrder, getNextHintUrutan } from "@/services/unsolvedCase";
import type { UnsolvedCaseHint, UnsolvedCaseHintType } from "@/types/unsolvedCase";
...
const HINT_TYPES: { value: UnsolvedCaseHintType; label: string; icon: React.ReactNode }[] = [
  { value: "chat", label: "Chat / Status", icon: <MessageSquare className="w-3.5 h-3.5" /> },
  { value: "karakter", label: "Karakter", icon: <User className="w-3.5 h-3.5" /> },
  { value: "buku", label: "Buku", icon: <Book className="w-3.5 h-3.5" /> },
  { value: "kartu", label: "Kartu", icon: <CreditCard className="w-3.5 h-3.5" /> },
  { value: "lainnya", label: "Lainnya", icon: <Box className="w-3.5 h-3.5" /> },
];

const TYPE_ICONS: Record<string, string> = {
  chat: "text-green-600",
  karakter: "text-blue-600",
  buku: "text-amber-600",
  kartu: "text-purple-600",
...
export default function UnsolvedCaseHintsEditor({ unsolvedCaseId }: Props) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const payload = { tipe: formTipe, konten: formKonten };
      if (editingHint) {
  ...
  const renderFormFields = () => {
    switch (formTipe) {
      case "chat": return renderChatFields();
      case "karakter": return renderKarakterFields();
      case "buku": return renderBukuFields();
      case "kartu": return renderKartuFields();
      case "lainnya": return renderLainnyaFields();
    }
  };
    ...
    <div className="space-y-4">
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-semibold border-2 transition-all flex items-center justify-center gap-1.5 ${
              formKonten.is_chat ? "border-[#4D455D] bg-[#F4F3FF] text-[#4D455D]" : "border-gray-200 text-gray-600"
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-semibold border-2 transition-all flex items-center justify-center gap-1.5 ${
              formKonten.is_chat === false ? "border-[#4D455D] bg-[#F4F3FF] text-[#4D455D]" : "border-gray-200 text-gray-600"
            }`}
          >
            <Image className="w-4 h-4" /> Bukan Chat
          </button>
        </div>
      </div>
      {formKonten.is_chat ? (
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          >
            <Plus className="w-3 h-3 inline mr-0.5" /> Tambah Gambar
          </button>
        </div>
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 border border-gray-300 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#9792EC] focus:border-transparent"
          />
          <FileUploader
            accept="image/*"
            label="Upload"
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          <button
            ...
            className="text-xs text-blue-600 hover:text-blue-800 font-semibold"
          >
            <Plus className="w-3 h-3 inline mr-0.5" /> Tambah Halaman
          </button>
        </div>
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          min={1}
          className="w-32 border border-gray-300 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#9792EC] focus:border-transparent"
        />
      </div>
    </div>
  );
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-6">
        <Loader2 className="w-5 h-5 animate-spin text-[#4D455D]" />
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
      <div className="flex items-center justify-between border-b border-gray-100 pb-3">
        <h2 className="font-bold text-lg text-gray-800 flex items-center gap-2">
          <Lightbulb className="w-5 h-5 text-amber-500" /> Hint
        </h2>
        <button
          type="button"
          onClick={openNew}
        ...
        >
          <Plus className="w-3 h-3" /> Tambah Hint
        </button>
      </div>
          ...
          {hints.map((hint, index) => (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <span className={`text-sm ${TYPE_ICONS[hint.tipe] || "text-gray-600"}`}>
                  {hint.tipe === "chat" ? <MessageSquare className="w-4 h-4" /> :
                   hint.tipe === "karakter" ? <User className="w-4 h-4" /> :
                   hint.tipe === "buku" ? <Book className="w-4 h-4" /> :
                   hint.tipe === "kartu" ? <CreditCard className="w-4 h-4" /> :
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-xl mx-4 max-h-[90vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {HINT_TYPES.map((t) => (
                    <button
                      key={t.value}
                      type="button"
                      onClick={() => { setFormTipe(t.value); setFormKonten({}); }}
                      className={`px-3 py-2 rounded-lg text-xs font-semibold border-2 transition-all flex items-center gap-1.5 justify-center ${
                        formTipe === t.value
                          ? "border-[#4D455D] bg-[#F4F3FF] text-[#4D455D]"
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-100 px-6 py-4 flex justify-end gap-3 rounded-b-2xl">
              ...
              <button
                ...
                onClick={handleSave}
                disabled={saving}
                className="px-6 py-2.5 bg-[#4D455D] text-white rounded-xl text-sm font-semibold hover:bg-[#3d364a] transition-colors disabled:opacity-50 flex items-center gap-1.5"
              >
                {saving ? (
