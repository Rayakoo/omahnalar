...
export default function FindWordGridEditor({ gridWidth, gridHeight, initialWords, onChange }: Props) {
  ...
  // Fire onChange after words are committed (never during render)
  useEffect(() => {
  ...
  // Build cell map: cellId → letter + wordId references
  ...
  // Check if a position is valid for a word
  ...
  // Find the first valid position for a word in the given direction
  ...
  const toggleDirection = useCallback((id: string) => {
    ...
    // Try current position first, then auto-find the best spot
  ...
  return (
    <div className="space-y-6">
              ...
              return (
                <button
                  ...
                  className={`relative w-8 h-8 rounded-sm border flex items-center justify-center text-xs font-bold transition-all ${
                    isSelected
                      ? "border-[#C2185B] bg-[#E75480] text-white shadow-md scale-110 z-10"
                      : cell
                        ? "border-brand-900 bg-[#E8F4FD] shadow-sm"
                        : "border-gray-200 bg-gray-50 hover:border-gray-400 cursor-pointer"
                  } ${isConflict ? "ring-2 ring-amber-400" : ""}`}
                >
                  <span className={`uppercase ${
                    isSelected ? "text-white" : cell ? "text-[#1a1a2e] font-extrabold" : "text-gray-300"
                  }`}>
                    {cell?.letter || "."}
                  </span>
                  {isStart && cell && cell.wordIds.length <= 1 && (
                    <span className="absolute -top-2 -right-2 text-[10px] font-black">
                      {words.find(w => cell.wordIds.includes(w.id) && w.row === ri && w.col === ci)?.direction === "across"
                        ? <span className="text-white bg-blue-600 rounded-full w-3.5 h-3.5 flex items-center justify-center"><ArrowRight className="w-2 h-2" /></span>
                        : <span className="text-white bg-emerald-600 rounded-full w-3.5 h-3.5 flex items-center justify-center"><ArrowDown className="w-2 h-2" /></span>
                      }
                    </span>
                  )}
                </button>
      ...
      <div className="flex items-center justify-between">
        <h4 className="font-bold text-sm">Kata dalam Grid</h4>
        <button
          type="button"
          onClick={addWord}
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3 pt-1">
                ...
                <div className="flex gap-1">
                  <button type="button" onClick={() => moveWord(w.id, -1, 0)} className="p-1 rounded hover:bg-gray-100 text-gray-500"><ArrowDown className="w-3 h-3 rotate-180" /></button>
                  <button type="button" onClick={() => moveWord(w.id, 1, 0)} className="p-1 rounded hover:bg-gray-100 text-gray-500"><ArrowDown className="w-3 h-3" /></button>
                  <button type="button" onClick={() => moveWord(w.id, 0, -1)} className="p-1 rounded hover:bg-gray-100 text-gray-500"><ArrowRight className="w-3 h-3 rotate-180" /></button>
                  <button type="button" onClick={() => moveWord(w.id, 0, 1)} className="p-1 rounded hover:bg-gray-100 text-gray-500"><ArrowRight className="w-3 h-3" /></button>
                </div>
                <div className="ml-auto">
                  ...
                  >
                    {w.direction === "across" ? <ArrowRight className="w-3 h-3" /> : <ArrowDown className="w-3 h-3" />}
