...
export default function TtsGridEditor({ initialCells, onChange }: Props) {
  ...
  // Track whether initialCells has been applied
  ...
  // Notify parent when cells change (skip initial mount)
  useEffect(() => {
  ...
  const updateCell = useCallback((id: string, letter?: string, clueIndex?: number, clueUpdate?: Partial<CellClue>) => {
    setCells(prev => {
      ...
      // If answer was updated, auto-fill/create cells in that direction
          ...
          if (clue) {
            ...
            for (let i = 0; i < maxLen; i++) {
              ...
              if (i < newAns.length) {
                ...
                // Clear cells beyond new answer length
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-center">
          <div className="relative inline-flex p-8 border-2 border-dashed border-gray-300 rounded-2xl">
            <button type="button" onClick={() => addCell(0, 0)} className="w-12 h-12 bg-[#3A3852] text-white rounded-xl hover:bg-[#4E4B6E] flex items-center justify-center shadow-md transition-all">
              ...
              if (!cell && hasNeighbor) {
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => addCell(r, c)}
                    className="w-10 h-10 rounded-lg border-2 border-dashed border-[#9792EC]/40 flex items-center justify-center hover:bg-[#9792EC]/10 hover:border-[#9792EC] transition-all group"
                    title="Tambah cell"
                  >
                    <Plus className="w-4 h-4 text-[#9792EC]/50 group-hover:text-[#9792EC]" />
                  </button>
                );
              }
                ...
                return (
                  <button
                    ...
                    className={`relative w-10 h-10 rounded-lg border-2 flex items-center justify-center font-bold text-base transition-all ${
                      selected === id
                        ? "border-[#E75480] ring-2 ring-[#E75480]/40 bg-[#E75480]/10 shadow-md scale-105"
                        : cell.clues.length > 0
                          ? "border-[#4D455D] bg-[#F5F0FF] shadow-md"
                          : cell.letter
                            ? "border-gray-300 bg-gray-100 shadow-sm"
                            : "border-gray-200 bg-gray-50 hover:border-gray-400"
                    }`}
                  >
                    <span className={`uppercase ${cell.letter ? "text-[#2C2C2C] font-extrabold" : "text-gray-300"}`}>
                      {cell.letter || "."}
                    </span>
                    {nums.length === 1 && (
                      <span className="absolute -top-[2px] -left-[1px] text-[10px] font-extrabold text-[#4D455D] pointer-events-none select-none leading-none bg-white/80 px-[2px] rounded-sm">
                        {nums[0]}
                      </span>
                    )}
                    {nums.length >= 2 && (
                      <span className="absolute -top-[2px] -left-[1px] text-[9px] font-extrabold text-[#4D455D] pointer-events-none select-none leading-tight text-left bg-white/80 px-[2px] rounded-sm">
                        {nums.slice(0, 2).join("/")}
                      </span>
                    )}
                  </button>
