...
export function setConfirmed(v: boolean) {
  localStorage.setItem(`${PREFIX}confirmed`, v ? "true" : "false")
}

export function getRevealedHints(caseId: string): Set<string> {
  if (typeof window === "undefined") return new Set()
  try {
    const raw = localStorage.getItem(`${PREFIX}revealed_${caseId}`)
...
export function addRevealedHint(caseId: string, hintId: string) {
  const s = getRevealedHints(caseId)
  s.add(hintId)
  localStorage.setItem(`${PREFIX}revealed_${caseId}`, JSON.stringify([...s]))
}
